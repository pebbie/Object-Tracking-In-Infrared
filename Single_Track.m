%NOTE: This is an automated version of Single_Track_Original, allowing for
%parameter input. All user interaction is also disable. This automated
%version is used in the optimization algorithm SetParm.
%-------------------------------------------------------------------------%
...
%
%Technique 2: Implements a Meanshift estimation of distribution centers
%based on the same preprocessed images as analyzed in Technique 1. 
%This can be turned on or off and is used primarily as a demonstration 
%tool. The meanshift tracking is done in the seperate Matlab function 
%camshift.m or camshift2.m. They were originally developed to implement
%meanshift tracking but were found to be no more effective than alternative
%techniques.
%-------------------------------------------------------------------------%

function [biglist parameters] = Single_Track(p1,p2,p3,p4,p5,p6,folder)
biglistcount = 1;
biglist = [];
...
o_size = 18;            %Minimum detectable object size
...
%figures
    ...
    I_db = imerode(I_bdiff,ones(p2,p3));
...
%-------------------------------------------------------------------------%
%Background Updating:
    %Update the background if a background frame
    ...
    biglist{biglistcount} = t_list; 
    biglistcount = biglistcount + 1;
end
toc
